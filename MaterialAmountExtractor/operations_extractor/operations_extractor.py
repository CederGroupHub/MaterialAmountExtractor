...
class OperationsExtractor:
    def __init__(self,
        ...
        print("Operations Extractor v2.4")
        ...
        for idx in verbs:
            ...
            if children != [] and [c for c in children[-1].children] != []:
                ...
                if word.pos_ == 'NOUN':
                    ...
                    fnn_prediction = fnn_model.predict_classes(np.array([embed_vec+gramma_vec]))[0]
    ...
    def __is_aqueous_mix(self, subtree):
    ...
    def find_aqueous_mixing(self, sentence, operations, parsed_tokens=False):
        ...
        if parsed_tokens:
            ...
            spacy_tokens = spacy.tokens.Doc(nlp.vocab, words=sentence)
            nlp.tagger(spacy_tokens)
            nlp.parser(spacy_tokens)

        mixing_operations = [(i, op_id) for i, (op_id, op_type) in enumerate(operations) if op_type == 'Mixing']
        mixing_operations.reverse()
        aqueous_mix = []
        if len(mixing_operations) > 0:
            prev_tree = []
            for op_id, mix in mixing_operations:
                mix_tree = [t.text for t in spacy_tokens[mix].subtree if t.text not in prev_tree]
                if spacy_tokens[mix].lemma_ in self.__aqueous_verbs or self.__is_aqueous_mix(mix_tree):
                    aqueous_mix.append(op_id)
                prev_tree = mix_tree

        for mix_id in aqueous_mix:
            op_id, op_type = updated_operations[mix_id]
            updated_operations[mix_id] = (op_id, 'AqueousMixing')
